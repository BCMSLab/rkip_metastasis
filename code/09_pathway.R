...
(list(MDA = oe, MCF7 = kd) %>%
  map_df(~{
    tibble(RAF1 = as.numeric(scale(.x$mat['RAF1',])),
           MAP2K7 = as.numeric(scale(.x$mat['MAP2K7',])),
           EPHB2 = as.numeric(scale(.x$mat['EPHB2',])),
           group = .x$group)
  }, .id = 'cell') %>%
  mutate(cell = ifelse(cell == 'MDA', 'MDA_MB_231', cell)) %>%
  gather(gene, expr, RAF1, MAP2K7, EPHB2) %>%
  group_by(cell, group, gene) %>%
  ...
  facet_grid(gene ~ cell, scales = 'free_x') +
  labs(x = 'Experimental group',
       y = 'Expression (standardized)') + 
  theme(panel.grid = element_blank(),
        axis.ticks.length=unit(.2, "cm"),
        axis.ticks = element_line(color= 'gray'),
        axis.title = element_text(size = 14))) %>%
  ggsave(plot = .,
         filename = 'output/manuscript/10_pathway.png',
         height = 5, width = 5)
